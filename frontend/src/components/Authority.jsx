...
import sendEmailToAuthority from "../utils/sendEmailToAuthority";
...
const Authority = () => {
  ...
  const closeAlert = async (id, userId, userEmail, userName, alertDetails) => {
    try {
      if (!userId) {
      ...
      // Call sendEmailToUser function to notify the user when the alert is closed
      const emailResponse = await sendEmailToAuthority({
        userEmail,
        userName,
        alertDetails,
      });
